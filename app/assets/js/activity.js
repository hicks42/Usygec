...
const newFormDeleteBtn = (newItem) => {
  ...
  const newRow = newItem.querySelector(".new-buttons");
...
document.querySelectorAll(".activity-buttons, .new-buttons").forEach((item) => {
  formSaveBtn(item);
});
...
const addGroupDeleteLink = (item) => {
  const removeFormButton = document.createElement("button");
  removeFormButton.innerHTML = '<i class="fa-solid fa-trash-can"></i>';
  removeFormButton.className = `col-1 btn btn-danger btn-sm`;
